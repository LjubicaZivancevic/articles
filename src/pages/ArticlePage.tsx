...
import { ArticleProps, ListOfArticles } from "../components/ArticlesTypes";

function ArticlePage() {
	const articles = useSelector((state: ListOfArticles) => {
		if (
			state.articles.articles !== undefined &&
			state.articles.searchTerm !== "" &&
			state.articles.searchTerm !== null &&
			state.articles.searchTerm !== undefined
		) {
			return state.articles.articles.filter((article: ArticleProps) =>
